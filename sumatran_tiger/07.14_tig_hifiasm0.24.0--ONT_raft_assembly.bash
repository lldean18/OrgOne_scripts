...
#SBATCH --job-name=tig_raft_hifiasm11
...
echo "running the raft / hifiasm pipeline with hifiasm version 0.24.0 with the --ont flag with the
input data: $reads
writing the results to the dir: $wkdir/raft_hifiasm_asm$attempt
"

####### STEP 1 ########
# run hifiasm on all duplex and error corrected (with herro) simplex reads to obtain error corrected reads and coverage estimate
hifiasm \
-o errorcorrect \
-t 96 \
--write-ec \
...
# inbetween new step to convert fasta to fastq so I can try with the --ont flag at the end
conda deactivate
conda activate seqtk1.4
seqtk seq -F '#' fragmented.reads.fasta > fragmented.reads.fastq
conda deactivate
conda activate hifiasm_0.24.0
...
-t 96 \
-r 1 \
fragmented.reads.fastq 2> finalasm.log
